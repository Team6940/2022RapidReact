...
import frc.robot.lib.team3476.tracking.TimestampedPose;
...
public final class RobotTracker extends SubsystemBase {
     ...
     */
    private  Pose2d latestRawPose = new Pose2d();
    ...
    private  ChassisSpeeds latencyCompensatedChassisSpeeds = new ChassisSpeeds();
    ...
    private final  SwerveDriveKinematics swerveDriveKinematics = SwerveDriveTrain.getInstance().kDriveKinematics;

    private final ReentrantReadWriteLock lock = new ReentrantReadWriteLock();

    private Translation2d acceleration = new Translation2d();

    private final  EvictingQueue<TimestampedPose> poseHistory = EvictingQueue.create(50);
    private final  EvictingQueue<ChassisSpeeds> chassisSpeedsHistory = EvictingQueue.create(12);


    private  Translation2d positionOffset = new Translation2d();

    private final LiveEditableValue<Double> visionUsagePercent = new LiveEditableValue<>(0.2,
            SmartDashboard.getEntry("Debug/RobotTracker/Vision Position Weight Percentage"));
...
//        }
    ...
    private void kalmanFilterFailed() {
        SmartDashboard.putNumber("Kalman Filter Failed Counter", failedTime.incrementAndGet());
    }
    ...
    public void periodic() {
        final SwerveDriveTrain drive = SwerveDriveTrain.getInstance();
        ...
        if (updateNextTick) {
            SwerveModuleState[] swerveModuleStates = drive.getStates();
...
//        }
    ...
    public void resetPosition( Pose2d pose,  Rotation2d gyroAngle) {
        ...
        try {
            ...
            latestChassisSpeeds = rotateChassisToFieldRelativeSpeeds(
                    drive.kDriveKinematics.toChassisSpeeds(drive.getStates()),
                    latestEstimatedPose.getRotation());
     ...
     */
    @Contract(pure = true)
    public  Pose2d getLastEstimatedPoseMeters() {
        lock.readLock().lock();
        try {
     ...
     */
    @Contract(pure = true)
    public  Pose2d getLatencyCompedPoseMeters() {
     ...
     */
    @Contract(pure = true)
    public  ChassisSpeeds getLatencyCompedChassisSpeeds() {
        lock.readLock().lock();
        try {
    ...
    public void outputTelemetry() {
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Pose X", getLastEstimatedPoseMeters().getX());
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Pose Y", getLastEstimatedPoseMeters().getY());
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Pose Angle", getLastEstimatedPoseMeters().getRotation().getRadians());
        SmartDashboard.putNumber("Debug/RobotTracker/Gyro Robot Pose Angle", getGyroAngle().getRadians());
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Velocity X", getLastChassisSpeeds().vxMetersPerSecond);
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Velocity Y", getLastChassisSpeeds().vyMetersPerSecond);
        SmartDashboard.putNumber("Debug/RobotTracker/Last Estimated Robot Velocity Theta", getLastChassisSpeeds().omegaRadiansPerSecond);

        SmartDashboard.putNumber("Debug/RobotTracker/Gyro Pitch", RobotTracker.getInstance().getGyro().getPitch());
        SmartDashboard.putNumber("Debug/RobotTracker/Gyro Pitch Velocity", gyroPitchVelocity);
        SmartDashboard.putNumber("Debug/RobotTracker/Gyro Roll", RobotTracker.getInstance().getGyro().getRoll());
        SmartDashboard.putNumber("Debug/RobotTracker/Gyro Roll Velocity", gyroRollVelocity);

        SmartDashboard.putNumber("Debug/RobotTracker/Min Gyro Roll", minGyroRoll);
        SmartDashboard.putNumber("Debug/RobotTracker/Max Gyro Roll", maxGyroRoll);

        SmartDashboard.putNumber("Debug/RobotTracker/Acceleration", acceleration.getNorm());
        SmartDashboard.putNumber("Debug/RobotTracker/Acceleration X", acceleration.getX());
        SmartDashboard.putNumber("Debug/RobotTracker/Acceleration Y", acceleration.getY());
